...
description: "AIGC率降低·困惑度重构员+思维痕迹注入器（Stage 5/7）。最大化 Perplexity，破坏高频 N-gram 搭配，并注入研究过程思维痕迹（以 L1 书面体表达，如'该结果与预期不符'/'其有效性可解释如下'）。基于 CNKI 实测：思维痕迹注入是最有效的 AIGC 降低手法之一。Triggers on: '困惑度', 'perplexity', 'N-gram破坏', '最终扫描', 'aigc降低-困惑度', 'aigc困惑度', '投稿就绪扫描', '思维痕迹'."
...
- ❌ 词汇必须符合顶刊的阅读习惯
- ❌ 必须扫完整篇——不许只改两段就交
- ❌ 思维痕迹注入必须用 L1 书面体——"该结果与预期不符" ✅；反问句、口号碎句、口语词、中文第一人称一律 ❌（详见 _REGISTER-SPEC.md）
...
4. **验证**：注入后逐句对照 _REGISTER-SPEC.md 禁用红线，确认无反问句、碎句、口语词、中文第一人称
