...
**带刀阿越设计的科研周报 skill** · 扫工程 → 对比上周 → 自动写周报
...
> 一个 周报 skill，专为科研组会设计：扫描你的工程目录、对比上周快照、问你简短的 L3 问卷，自动写出可直接交导师的 PhD 风格周报（Markdown + PDF）。

## 为什么要用

作为PhD学生党 一周里产生的代码、实验结果、论文草稿、理论推导、配图量大到周五完全记不清。盯着 50 个文件改动问自己「这周到底干了啥」是常态。

这个 skill 替你回答。它扫工程目录、对比上周快照、识别同 family 内推进的版本链（如 `xxx_v3.py → xxx_v4.py`）、跨 seed 聚合指标（mean ± std）、抽 paper 里的新公式块，只问你它推不出来的语义部分。然后写出结构化的导师周报。
...
# 2. 扫工程（万级数据文件秒级出 manifest）
# 3. 对比上周
# 4. 打开 interview.md，5-9 个 section（这一部分要自己去选，已经做了UI设计了，直接做选择题就好）
...
- `[project] advisor = "X老师"` —— 用于问候
- `[project] student = "带刀阿越"` —— 用于结尾签名
...
F526实验室内部工具。在你课题组的 git 仓库里维护、一起迭代。公开发布时请保留作者信息。大家一起优化
